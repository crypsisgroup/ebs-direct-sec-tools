...
func checkContentsRegex(b []byte) []string {
	...
	if re_api_key.Find(b) != nil {
		// this file has a hit!, make sure we record this!
        fmt.Println("Finding: AWS Account Key [Low Fidelity Signature]")
        fmt.Println("Finding Data:\n")
        fmt.Printf(string(re_api_key.Find(b)))
        regex_results = append(regex_results, "re_api_key")
    }
	if re_slack_api_key.Find(b) != nil {
		// this file has a hit!, make sure we record this!
        fmt.Println("Finding: AWS Account Key [Medium Fidelity Signature]")
        fmt.Println("Finding Data:\n")
...
func main() {
    ...
    bufferwrite := flag.Bool("dumpbytes",false,"Whether or not to dump the bytes in question to disk. It'd dump the second bytes, not their original values. It'll be called diff-snap-xxxx in the current directory.")
    flag.Parse()
    ...
    process_snapshot(*snapid, *bar, *region, *bufferwrite)
}